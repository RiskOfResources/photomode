...
public class PhotoModeSettings {
   ...
   public PhotoModeSettings(ConfigFile configFile, string baseDir) {
      ...
      NumberOfDollyPoints = CreatePhotoModeSetting(SettingCategory.General, "Dolly Path Smoothing", 150, "How many line segments to break the dolly path into to create a smooth path. Only applies when you have at least 1 checkpoint or with the auto dolly.", min: 2, max: 500, increment: 1);
      AutoDollyPathLength = CreatePhotoModeSetting(SettingCategory.General, "Auto Dolly Path Length", 10f, "How many nodes to visit in the auto dolly path. If you set this above 24 it will automatically use the fast auto dolly path generation.", min: 1, max: 100, increment: 1);
